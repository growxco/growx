...
export function createReconcileHandler({
  claimReconciliationLeaseImpl = claimReconciliationLease,
  estadoDoLoteImpl = estadoDoLote,
  reconcileExpiredHoldsImpl = reconcileExpiredHolds,
  reconcileStripeCanonicalSessionImpl = reconcileStripeCanonicalSession,
  reconcileMercadoPagoPaymentByIdImpl = reconcileMercadoPagoPaymentById,
  reconcileMercadoPagoOrderByIdImpl = reconcileMercadoPagoOrderById,
} = {}) {
  return async function handler(req, res) {
    ...
    const now = new Date();
    try {
      const lease = await claimReconciliationLeaseImpl({ now });
      if (!lease.acquired) {
        return res.status(200).json({ ok: true, executado: false, motivo: 'lease_existente' });
      }

      const reconciliation = await reconcileExpiredHoldsImpl({
        now,
        ...
        reconcileStripePaidImpl: ({ session, fetchImpl, deadlineAt }) => (
          reconcileStripeCanonicalSessionImpl(process.env.STRIPE_SECRET_KEY, session, {
            fetchImpl,
            deadlineAt,
          })
        ),
        reconcileMercadoPagoPaidImpl: ({ payment, fetchImpl, deadlineAt }) => (
          reconcileMercadoPagoPaymentByIdImpl(process.env.MP_ACCESS_TOKEN, String(payment.id), {
            fetchImpl,
            deadlineAt,
          })
        ),
        reconcileMercadoPagoOrderPaidImpl: ({ order, fetchImpl, deadlineAt }) => (
          reconcileMercadoPagoOrderByIdImpl(process.env.MP_ACCESS_TOKEN, String(order.id), {
            fetchImpl,
            deadlineAt,
          })
        ),
      });
      const lote = await estadoDoLoteImpl({ now });
